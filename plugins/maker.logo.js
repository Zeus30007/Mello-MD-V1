let handler = async (m, { conn, args, text, usedPrefix, command }) => {
    ...
    let pp = await conn.profilePictureUrl(m.sender, 'image').catch(_ => 'https://telegra.ph/file/c0b06bf53c9e2ab5a1eb7.jpg')
    let user = global.db.data.users[m.sender]
   let tee = `✳️ Enter a short text\n\n📌 Example  : *${usedPrefix + command}* ᴍᴇʟʟᴏ`
   let too = `✳️ Separate the text with a *+* \n\n📌 Example : \n*${usedPrefix + command}* ᴍᴇʟʟᴏ`
    m.react(rwait)
